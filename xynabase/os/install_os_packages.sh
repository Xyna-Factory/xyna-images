...
if [[ ${OS_IMAGE} == oraclelinux:* ]]; then
    ...
    apt -y update
    apt -y upgrade
    apt-get -y install zip unzip patch wget netcat-traditional xinetd net-tools bind9utils vim less dc libxml2-utils gnupg ca-certificates curl gcc python3-dev python3-venv python3-pip systemd
